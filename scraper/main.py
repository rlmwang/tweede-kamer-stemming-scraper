...
def parse_motie_info(url: str, soup) -> dict:
    ...
    all_doc_nrs = [span.next_sibling.strip() for span in doc_nr_spans if 'Nummer:' in span.get_text()]
...
def parse_motie_page_(url):
    ...
    while supporter_info_0.next_sibling.next_sibling is not None:
        ...
        indieners_table = indieners_table.append({'document_nr': doc_number, 'name_submitter': raw_individual_info[5], 'submitter_type': raw_individual_info[4], 'party_submitter': raw_individual_info[7], 'personal_page': 'https://www.tweedekamer.nl{}'.format(supporter_info_0.find('a')['href'])}, ignore_index=True)
    ...
    if page.find('table', class_='vote-result-table') is None:
        ...
        for table in tables:
            ...
            for party in parties[1::]:
                ...
                stemming_table = stemming_table.append({'document_nr': doc_number, 'party_name': party_name.replace('\n', ''), 'vote_count': count_vote, 'vote': choice}, ignore_index=True)
    ...
    if page.find('h2', string="Activiteiten"):
        ...
        if len(cards) > 0:
            for x in cards:
                ...
                activities_table = activities_table.append({'document_nr': doc_number, 'activities': activities_url},
                                                           ignore_index=True)

    motie_table = motie_table.append({'document_nr': doc_number, 'Subject': subject, 'Date': date, 'Text': motion_text, 'Title': page_title, 'State_Document': state_doc}, ignore_index=True)
    return motie_table, indieners_table, stemming_table, activities_table
...
def run(begin_page, end_page=None):
    ...
    for i in tqdm(range(begin_page, end_page)):
        ...
        for link in links:
            ...
            data = parse_stemming_page(
                url=DEBAT_URL.format(link=link.strip('/'))
            )
...
def merge_tables(data_a: dict[str, pl.DataFrame], data_b: dict[str, pl.DataFrame]) -> dict[str, pl.DataFrame]:
    output = {}
